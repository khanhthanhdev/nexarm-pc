...
# Embedded file name: nexarm_qt\ui\ai_calibration_widget.py
import json, os, re, struct
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QDoubleSpinBox, QGroupBox
...
class AICalibrationWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.btn_start.clicked.connect(lambda: (self.send_cmd(1), self._calib_active(self.btn_start)))
        self.btn_grab = QPushButton(STRINGS[self.lang].get("btn_calib_grab", "2. Test Grab"))
        self.btn_grab.clicked.connect(lambda: (self.send_cmd(2), self._calib_active(self.btn_grab)))
        self.btn_reset = QPushButton(STRINGS[self.lang].get("btn_calib_reset", "3. Reset"))
        self.btn_reset.clicked.connect(lambda: (self.send_cmd(3), self._calib_inactive()))
        self.btn_stop = QPushButton(STRINGS[self.lang].get("btn_calib_stop", "4. Stop"))
        self.btn_stop.clicked.connect(lambda: (self.send_cmd(0), self._calib_inactive()))
    ...
    def process_line(self, line):
        try:
            if "[校准]" in line:
                match_center = re.search("X偏:(-?\\d+) Y偏:(-?\\d+)", line)
                if match_center:
                    ...
                    match_dev = re.search("\\[校准\\] (.*?) (\\d+) 像素 \\| (.*?) (\\d+) 像素", line)
                    if match_dev:
    ...
    def send_cmd(self, sub_cmd):
        self.comm_manager.send_packet(255, 49, [sub_cmd])
    ...
    def save_offsets(self):
        ...
        self.comm_manager.send_packet(255, 45, data_list + [0, 0])
        try:
