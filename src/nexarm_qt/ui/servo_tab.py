...
class ServoTab(QWidget):
    ...
    def setup_ui(self):
        ...
        for i in range(6):
            ...
            w.spin_pos.valueChanged.connect(lambda v, x=i: self.on_spin_pos_change(x, v))
            w.spin_acc.valueChanged.connect(lambda v, x=i: self.update_params(x, acc=v))
            w.spin_spd.valueChanged.connect(lambda v, x=i: self.update_params(x, spd=v))
            w.btn_torque.toggled.connect(lambda checked, x=i: self.send_servo_torque(x, 1 if checked else 0))
