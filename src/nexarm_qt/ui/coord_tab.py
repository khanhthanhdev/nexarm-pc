...
# Embedded file name: nexarm_qt\ui\coord_tab.py
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QGroupBox, QPushButton, QLabel, QDoubleSpinBox, QSpinBox, QGridLayout, QFrame, QSplitter
    ...
    class DirectionWheel(QWidget):
        ...
        def paintEvent(self, event):
            ...
            for i in range(4):
                ...
                tx = center.x() + dist * math.cos(mid_rad)
                ty = center.y() - dist * math.sin(mid_rad)
        ...
        def get_segment_at(self, pos):
            ...
            if dist < 40 or dist > self.width() // 2:
                return
    ...
    class CoordTab(QWidget):
        ...
        def setup_ui_manual(self):
            ...
            if HAS_3D:
                ...
                self.btn_3d_view.setStyleSheet("color: #FA8F01; border: 1px solid #FA8F01; padding: 8px 24px; font-size: 11pt; font-weight: bold; border-radius: 4px; background: transparent; min-height: 24px;")
            ...
            self.sb_step.setRange(1, 100)
            self.sb_step.setValue(10)
            ...
            self.sb_speed.setRange(10, 2000)
            self.sb_speed.setValue(20)
            ...
            for idx, (k, v) in enumerate(zip(items, vals)):
                ...
                self.btn_home.clicked.connect(lambda: self.comm_manager.send_sys(CMD_ARM_RESET, [232, 3]))
        ...
        def handle_wheel_click(self, cmd):
            if cmd == "RESET":
                ...
                if "X" in cmd:
                    self.adjust_coord("X", 1 if "+" in cmd else -1)
                else:
                    if "Y" in cmd:
                        self.adjust_coord("Y", 1 if "+" in cmd else -1)
        ...
        def update_coord_display(self, x, y, z, p=0.0, r=0, c=0.0, servos=[]):
            if hasattr(self, "coord_value_labels"):
                vals = {'X':x, 
                 'Y':y,  'Z':z,  'Pitch':f"{p:.1f}",  'Roll':r,  'Claw':f"{c:.1f}"}
        ...
        def update_language(self, lang):
            self.lang = lang
            if hasattr(self, "lbl_ik_title"):
                self.lbl_ik_title.setText(STRINGS[lang]["grp_ik"])
            if hasattr(self, "grp_params"):
                self.grp_params.setTitle(STRINGS[lang]["grp_coordinates"])
            if hasattr(self, "btn_send"):
                self.btn_send.setText(STRINGS[lang]["btn_send_coord"])
            if hasattr(self, "grp_stat"):
                self.grp_stat.setTitle(STRINGS[lang]["grp_realtime_stat"])
            if hasattr(self, "btn_get"):
                self.btn_get.setText(STRINGS[lang]["btn_get_coord"])
            if hasattr(self, "btn_home"):
                self.btn_home.setText(STRINGS[lang].get("btn_home_all", "全部回中"))
            if hasattr(self, "btn_read_servos"):
                self.btn_read_servos.setText(STRINGS[lang].get("btn_read_servos", "读取舵机"))
            if hasattr(self, "arm_3d"):
                if self.arm_3d is not None:
                    self.arm_3d.update_language(lang)
            if hasattr(self, "btn_3d_view"):
                self.btn_3d_view.setText(STRINGS[lang].get("btn_open_3d", "3D 视图"))
            if hasattr(self, "_3d_window"):
                if self._3d_window is not None:
