...
# Embedded file name: nexarm_qt\ui\peripheral_tab.py
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QGroupBox, QPushButton, QLabel, QSpinBox, QComboBox, QGridLayout, QLineEdit, QScrollArea, QSlider, QFrame, QSizePolicy
...
class PeripheralTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.ent_belt.setRange(-100, 100)
        self.ent_belt.setValue(50)
        r1.addWidget(self.ent_belt)
        conv_lay.addLayout(r1)
        r2 = QHBoxLayout()
        self.btn_conv_run = QPushButton(STRINGS[self.lang].get("btn_run", "运行"))
        self.btn_conv_run.clicked.connect(lambda: (self.comm_manager.send_sys(CMD_CONVEYOR_SET, [self.ent_belt.value() & 255]), self._flash(self.btn_conv_run)))
        ...
        self.ent_step.setRange(-100000, 100000)
        self.ent_step.setValue(2000)
        ...
        self.cb_step_div.addItems(['1', '2', '4', '8', '16', '32'])
        self.cb_step_div.setCurrentIndex(2)
        ...
        self.spin_channel.setRange(0, 14)
        ...
        self.spin_sync_acc = QSpinBox()
        self.spin_sync_acc.setRange(0, 254)
        ...
        self.btn_scan_ch = QPushButton(STRINGS[self.lang].get("btn_scan_channels", "扫描信道"))
        ...
        self.dpad_mec = DPad(mode="6dir", lang=(self.lang))
        ...
        self.dpad_tank = DPad(mode="4dir", lang=(self.lang))
    ...
    def send_stepper_div(self):
        ...
        div_map = {
         1: 0, 2: 1, 4: 2, 8: 3, 16: 7, 32: 7}
        try:
            div = int(self.cb_step_div.currentText())
            code = div_map.get(div, 0)
    ...
    def _send_mec_combined(self):
        ...
        for name in self.dpad_mec._active_set:
            ...
            self.comm_manager.send_sys(CMD_MECANUM_RUN, [vx & 255, vy & 255, vz & 255])
    ...
    def _on_tank_pressed(self, name):
        ...
        if name in mapping:
            s, t = mapping[name]
            self.comm_manager.send_sys(CMD_TANK_RUN, [s & 255, t & 255])
    ...
    def _on_motor_slide(self, idx, val):
        ...
        self.comm_manager.send_sys(CMD_SET_SINGLE_MOTOR, [idx + 1, val & 255])

    def send_multi_motors(self):
        try:
            speeds = [sp.value() & 255 for sp in self.multi_mot_vars]
    ...
    def _flash(self, btn):
        ...
        btn.setText(STRINGS[self.lang].get("msg_set_success", "OK"))
    ...
    def update_language(self, lang):
        ...
        self.btn_scan_ch.setText(S.get("btn_scan_channels", "Scan Channels"))
